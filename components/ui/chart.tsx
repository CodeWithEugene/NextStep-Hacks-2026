...
function ChartLegendContent({
  ...
  nameKey,
}: React.ComponentProps<"div"> & { payload?: ReadonlyArray<{ value?: string; dataKey?: string | number; color?: string }>; verticalAlign?: "top" | "bottom"; hideIcon?: boolean; nameKey?: string }) {
  const { config } = useChart();
  ...
  return (
    <div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key);
        return (
          <div key={String(item.value)} className="[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3">
